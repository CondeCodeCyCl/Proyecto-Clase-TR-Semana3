...
import com.carlos.commons.dto.ErrorResponse;
...
public class GlobalHandlerException {
	...
	public ResponseEntity<ErrorResponse> handleConstraintViolationException(ConstraintViolationException e) {
		log.error("Violaccion de restriccion: {}", e.getMessage());
	...
	/*
	 * Argumento mal escrito por el cliente
	 */
	...
	public ResponseEntity<ErrorResponse> handleGenericFeignException(FeignException e) {
		...
		String remoteMessage = e.contentUTF8();
		String finalMessage;

		if (remoteMessage != null && !remoteMessage.isEmpty()) {
			try {
				ObjectMapper mapper = new ObjectMapper();
				JsonNode node = mapper.readTree(remoteMessage);
				finalMessage = node.has("mensaje") ? node.get("mensaje").asText() : remoteMessage;
			} catch (Exception ex) {
				finalMessage = remoteMessage;
			}
		} else {
			// 3. Si no hay mensaje remoto, usamos tu switch de respaldo
			finalMessage = switch (status) {
		...
		ErrorResponse response = new ErrorResponse(status, finalMessage);
	...
	public ResponseEntity<ErrorResponse> handleIllegalStateException(IllegalStateException e) {
		log.warn("Conflicto de negocio: {}", e.getMessage());
		return ResponseEntity.status(HttpStatus.CONFLICT)
				.body(new ErrorResponse(HttpStatus.CONFLICT.value(), e.getMessage()));
	}
}
